...
目前系統支援四種 Widget 類型，每種支援的 `props` 不同。
請注意：大部分 Widget 的顏色 (`color`) 目前是由程式碼固定或根據數值自動變色，**設定檔中的 `color` 參數大多會被忽略**。
...
| **Config** | `props.color` | ❌ 忽略 | 目前寫死使用 `var(--text-primary)`。      |
| **Config** | `props.unit`  | ❌ 忽略 | 需直接包含在 `value` 字串中。             |
...
| :--------- | :------------ | :------ | :--------------------------------------------- |
| **Config** | `title`       | ✅ 支援 | 左上角標題。                                   |
| **Config** | `props.color` | ❌ 忽略 | 進度條顏色自動根據 `percent` 決定 (綠/黃/紅)。 |
