...
function App() {
  ...
  const migrateLayouts = useCallback((cfg: AppConfig): AppConfig => {
    ...
    const isOldFormat = cfg.widgets.every((w) => !w.layout || (w.layout.w <= 2 && w.layout.h <= 2))
  ...
  const loadConfig = useCallback(async () => {
    try {
      ...
      debugLog(
        "INFO",
        "Config",
        `Loaded — ${cfg.widgets.length} widgets, opacity=${(cfg.opacity || 0.72).toFixed(2)}, mode=${cfg.windowMode || "normal"}`
      )
  ...
  const loadModules = () => {
    SystemService.GetModules()
      .then((infos: any) => {
        ...
        if (isInitialLoadRef.current) {
          debugLog(
            "INFO",
            "Modules",
            `Loaded — ${parsed.length} total, ${parsed.filter((m) => m.enabled).length} enabled`
          )
      ...
      .then((data: any) => {
        if (data) {
          const snapshot: Record<string, DataPayload> =
    ...
    SystemService.GetConfig()
      .then((cfg: AppConfig) => {
        setAppConfig((prev) => {
          ...
          const mergedWidgets: WidgetConfig[] = cfg.widgets.map((w) => ({
            ...w,
  ...
  useEffect(() => {
    ...
    if (!isEditMode && layouts && config) {
      ...
      SystemService.SaveConfig(newConfig)
        .then(() =>
          debugLog("INFO", "Layout", `Saved — ${newWidgets.filter((w) => w.layout).length} widgets`)
        )
  ...
  const handleDrag = useCallback(
    (_layout: Layout, _oldItem: any, _newItem: any, placeholder: any) => {
      if (placeholder) {
          ...
          >
            ...
            >
              {isSettingsOpen ? (
                ...
                <>
                  GLANCEHUD{" "}
                  <span style={{ opacity: 0.5, fontSize: "0.9em", marginLeft: 4 }}>
